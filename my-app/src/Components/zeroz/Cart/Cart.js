...
export default function Cart() {
  ...
  const [shoesTotal, setShoesTotal] = useState(() => {
    return JSON.parse(window.localStorage.getItem("coupon"))
      ? JSON.parse(window.localStorage.getItem("coupon")) *
          ...
          +editProduct.reduce(
            (acc, item) => acc + +item.price.new.replace("$", ""),
            0
          )
      : shoesNum.reduce((acc, item) => acc + item, 0) *
          +editProduct.reduce(
  ...
  const [discount, setDiscount] = useState(() =>
    JSON.parse(window.localStorage.getItem("discount")) ? true : false
  );
  useEffect(() => {
    window.addEventListener("storage", () => {
      setShoesTotal(
        editProduct.reduce((acc, item) => {
          const price = parseFloat(item.price.new) * item.shoesNumber || 0; // Default to 0 if invalid
          return acc + price;
        }, 0)
      );
    });
    setShoesNum((prevShoesNum) => {
      ...
      if (savedItems.length > prevShoesNum.length) {
        return [
          ...prevShoesNum,
          ...new Array(savedItems.length - prevShoesNum.length).fill(1),
        ];
  ...
  function changeShoesNum(e) {
    ...
    setShoesNum((prevShoesNum) => {
      return prevShoesNum.map((item, index) =>
        index == curruntSliderId ? Math.max(item + adjustment, 1) : item
      );
    });
    window.dispatchEvent(new Event("storage"));
  }
  useEffect(() => {
    const updatedProducts = JSON.parse(window.localStorage.getItem("item")).map(
      (item, index) => ({
        ...item,
        shoesNumber: shoesNum[index],
      })
    );
  ...
  function deleteElement(e) {
    ...
    if (JSON.parse(window.localStorage.getItem("item").length !== 0)) {
      const updatedShoppingElements = editProduct.filter(
  ...
  return (
    <div className="mainDiv bg-[#f1f1ef] flex justify-center items-center h-[fit-content] px-[12%] py-[2%] min-w-fit">
      <div className="bg-white w-[100%] h-[100%] max-h-fit my-[3%] px-[5%] min-w-fit">
        <h1 className="text-7xl font-semibold text-center py-[5%] border-[#6e7051] border-b-4">
          Cart
        </h1>
        {editProduct.length != 0 ? (
          <div className="tableWrapper max-h-fit flex gap-x-5 h-[100%] mt-2 mb-[2%]">
            <table className="table-auto w-[70%] h-[20%] border">
              <>
                ...
                {editProduct.map((item, index) => (
                  <tr key={index} className="tableRow border">
                    <td className="information flex items-center p-[3%] gap-x-[10%] ms-[5%] h-[100%]">
                      <p
                        onClick={deleteElement}
                        className="cartClosingBtn text-2xl text-[black] cursor-pointer"
                      >
                        x
                      </p>
                    ...
                    <td className="totalPrice font-semibold text-[black] text-center">
                      {
                        <div className="txtContainer flex justify-between">
                          ...
                          {item.price.new == item.price.old ? (
                            <p className="txtValue">${item.price.old}</p>
                          ) : (
                    ...
                    <td className="sliderContainer flex justify-between">
                      ...
                      <div className="flex h-fit justify-center text-[black]">
                        <div
                          onClick={changeShoesNum}
                          className="select-none border px-3 pb-1 text-3xl cursor-pointer"
                        >
                          -
                        </div>
                        <div
                          id={index}
                          className="border px-3 pt-1.5 text-xl select-none"
                        >
                          {cartProducts ||
                          (shoesNum[index] == item.shoesNumber && shoesNum)
                            ? shoesNum[index]
                            : item.shoesNumber}
                        </div>
                        <div
                    ...
                    <td className="productSubtotal txtContainer text-center text-[black] font-semibold">
                      <div className="flex">
                        <p className="txtTitle hidden w-full">
                          Product subtotal
                        </p>
                        <p className="txtValue">
                          $
                          {cartProducts || shoesNum[index] == item.shoesNumber
                            ? parseFloat(
                                shoesNum[index] *
                                  item.price.new.replace("$", "")
                              ).toFixed(2)
                            : parseFloat(
                                item.shoesNumber *
                                  item.price.new.replace("$", "")
            ...
            <div className="w-[30%] mb-[2%] h-[fit-content] border">
              <h4 className="bg-[#f1f1ef] h-[8vh] text-2xl font-semibold flex items-center px-[7%]">
                Cart totals
              </h4>
              <div className="cartTotalContainer px-[7%] pb-[5%] text-lg text-[#a0a3a4] font-semibold">
                <div className="flex border-b justify-between py-5">
                  ...
                  <h6 className="txtValue font-semi-bold text-black">
                    ${shoesTotal.toFixed(2) || null}
                  </h6>
                </div>
                <div className="flex border-b justify-between items-center py-5">
                  ...
                  <h6 className="txtValue font-semi-bold text-black">
                    {discount ? (
                      <div className="">
                        <p className="line-through me-[10%] text-gray-300">
                          ${shoesTotal.toFixed(2) || null}
                        </p>
                ...
                {coupon ? (
                  <div className="AddCoupon w-[100%] flex gap-x-[3%] min-w-[fit-content] mt-[25%]">
                    <input
                      id="coupon"
                      className="border h-[fit-content] w-[70%] p-3 text-[1rem] text-black"
                      type="text"
                      placeholder="Coupon code"
                    />
                    <button
                      onClick={addCoupon}
                      className="bg-[#6e7051] text-[1rem] w-[40%] text-white duration-[0.8s] hover:bg-[#262b2c]"
                    >
                      APPLY
                    </button>
                  </div>
                  ...
                  <p
                    ...
                    className="coupon cursor-pointer mt-[25%] text-black"
                  >
                    Have a coupon?
                  </p>
                )}
                <a href="/checkout">
                  <button className="bg-[#6e7051] mt-[5%] text-white p-5 w-[100%] duration-[0.8s] hover:bg-[#262b2c]">
                    PROCEED TO CHECKOUT
                  </button>
                </a>
              </div>
          ...
          <div className="mt-[1%] mb-[10%]">
            <div className="flex p-[3%] gap-x-[2%] bg-[#eee] ">
              <FontAwesomeIcon
                className="text-2xl mt-1.5 text-[#6e7051]"
                icon={faDolly}
              />
              <p className="text-xl">Your cart is currently empty.</p>
            </div>
            <a href="/">
              <button className="bg-[#6e7051] text-[1.4rem] mt-[2%] text-white p-5 w-[100%] duration-[0.8s] hover:bg-[#262b2c]">
                Return to shop
              </button>
            </a>
          </div>
